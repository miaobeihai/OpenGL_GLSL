...
public:
    ...
    void    initialize()
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, 5 * sizeof(float), 0, GL_STATIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    ...
    void    beginFeedback()
    {
        ...
        {
            glBindTransformFeedback(GL_TRANSFORM_FEEDBACK, _feedbackId);
            glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, _sphereVBO);

            glBeginTransformFeedback(GL_POINTS);
            glBindBuffer(GL_ARRAY_BUFFER,0);
            glVertexAttribPointer(_shaderFeedback._position,  3, GL_FLOAT, GL_FALSE, sizeof(CELL::float3),   testData);
            ...
            glGetBufferSubData(GL_TRANSFORM_FEEDBACK_BUFFER, 0, 5 * sizeof(int), res);
            glBindTransformFeedback(GL_TRANSFORM_FEEDBACK, 0);
        }
        _shaderFeedback.end();
