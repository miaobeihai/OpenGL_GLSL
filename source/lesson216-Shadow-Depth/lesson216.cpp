...
#include "ShaderDepth.hpp"

class   SamplerExt :public OpenGLWindow
{
public:
    ShaderLight     _shader;
    ShaderDepth     _shaderDepth;
    Camera          _camera;
    bool            _lButtonDown;
    bool            _rButtonDown;
...
public:
    ...
    void    onInitGL()
    {
        ...
        _camera.setEye(CELL::real3(0,5,0));
        _camera.update();
        _modelMgr._program.initialize();
        _shader.initialize();
        _shaderDepth.initialize();
        ...
        sprintf(resPath, "%s../data/Detail.bmp", binPath);
        _texDetail  =   createTextureFromImage(resPath);

        sprintf(resPath, "%s../data/test1n.jpg", binPath);
        ...
        _texCube    =   createCubeTextureFromFile(cubeFile,type);
        _fbo.setupOnlyDepth(_width,_height);
    }
    void    render()
    {
        {
            ...
            _camera.perspective(45, (float)_width / (float)_height, 0.1, 30);
        ...
        CELL::matrix4   orth    =   CELL::ortho<float>(0.0f, 1.0f, 0.0f, 1.0f, 0.0f, 1.0f);
        glMatrixMode(GL_PROJECTION);
        glLoadMatrixf(orth.data());
        ...
        glActiveTexture(GL_TEXTURE0_ARB);
        glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, _fbo._texDepth);
        ...
        _shaderDepth.begin();
        {
            glUniform1i(_shaderDepth._tex, 0);
            glUniformMatrix4fv(_shaderDepth._mvp, 1, GL_FALSE, orth.data());

            glVertexAttribPointer(_shaderDepth._position,    3, GL_FLOAT, GL_FALSE, sizeof(VertexPUV), rect);
            glVertexAttribPointer(_shaderDepth._uv,          2, GL_FLOAT, GL_FALSE, sizeof(VertexPUV), &rect[0].u);
    ...
    void    renderFBO()
    {
        _fbo.begin();
        {
            ...
            _camera.setViewSize(_width, _height);
            _camera.perspective(45, (float)_width / (float)_height,1, 30);
            CELL::matrix4   mvp = _camera.getProject() * _camera.getView();
...
protected:
    ...
    GLuint      createCubeTextureFromFile( char* image[],  int target[])
    {
        GLuint  texId   =   0;
        ...
        {
            char    szBinPath[1024];
            getResourcePath(0, szBinPath);
            char    resPath[1024];
            sprintf(resPath, "%s../data/cube/%s", szBinPath,image[i]);
            stbi_uc*    pixels = stbi_load(resPath, &w, &h, &chanel, 0);
            if (chanel == 3)
            {
                glTexImage2D(target[i], 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, pixels);
            }
            else
            {
                glTexImage2D(target[i], 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
            }
            free(pixels);
        }
        glBindTexture(GL_TEXTURE_CUBE_MAP, 0);
