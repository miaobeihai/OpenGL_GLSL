...
class   ShaderVertex1 :public GLSLProgram
{
public:
    attribute   _position;
    attribute   _color;
    ...
    virtual void    initialize()
    {
        const char* vs  =   "#version 430 \n\
                            ...
                            {\n\
                                _outColor   =   _color;\n\
                                _outUV      =   _uv;\n\
                                vec3    posNew   =  _position + vec3(220,0,0);\n\
                                gl_Position =   _mvp * vec4(posNew,1.0);\n\
                            }";
        const char* ps =    "#version 430 \n\
                            layout(location = 0)  in    vec3    _inColor;\n\
                            layout(location = 1)  in    vec2    _inUV;\n\
                            ...
                            {\n\
                                vec4    color = texture2D(_tex,_inUV);\n\
                                gl_FragColor = color * vec4(_inColor,1) * vec4(1,0,0,1);\n\
                            }";
        createProgram(vs, ps);
